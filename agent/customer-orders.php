...
$stmt = $pdo->prepare("SELECT customer_order.id AS id ,customer_order.created_at AS created_at, customer_order.status AS status, customer.name AS name, orderandproduct_customer.quantity AS quantity, customer_order.amount AS amount FROM customer_order INNER JOIN user AS customer ON customer.id = customer_order.customer INNER JOIN orderandproduct_customer ON orderandproduct_customer.order_id = customer_order.id WHERE customer_order.status NOT IN ('pending','canceled','success') AND customer.state = ? AND customer_order.agent = ?");

$stmt->execute([$user_state, $user_id]);
$orders = $stmt->fetchAll(PDO::FETCH_ASSOC);


if (isset($_POST['mark_as_delivered'])) {
    $order_id = $_POST['id'];
    $stmt = $pdo->prepare("UPDATE customer_order SET status=:status,agent_delivered=:agent_delivered WHERE id=:order_id");
    $stmt->execute([
...
if (isset($_POST['cancel_order'])) {
    $order_id = $_POST['id'];
    $stmt = $pdo->prepare("UPDATE customer_order SET status=:status, agent=:agent_id,agent_delivered=:agent_delivered WHERE id=:order_id");
    $stmt->execute([
        'status' => "pending",
        'agent_id' => null,
...
<head>
    ...
    <!-- Page Title  -->
    <title>Agent | Customer orders</title>
    <!-- StyleSheets  -->
                                            ...
                                            <?php foreach ($orders as $order) : ?>
                                                <div class="nk-tb-item">
                                                    ...
                                                    <div class="nk-tb-col tb-col-sm">
                                                        <span class="tb-sub"><?php echo ucwords($order['name']) ?></span>
                                                    </div>
                                                    <div class="nk-tb-col tb-col-md">
                                                            ...
                                                            if ($order['status'] == 'agent_accepted') { ?>
                                                                <li>
                                                                    <form method="post">
                                                                        ...
                                                                        <button type="submit" name="mark_as_delivered" class="btn btn-icon btn-trigger btn-tooltip" aria-label="Mark as Delivered" data-bs-original-title="Mark as Delivered">
