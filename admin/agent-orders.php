...
$stmt = $pdo->prepare("SELECT agent_order.id,agent_order.created_at,agent_order.amount,agent_order.status,agent.name AS agent_name,SUM(orderandproduct_agent.quantity) AS quantity FROM orderandproduct_agent INNER JOIN agent_order ON agent_order.id = orderandproduct_agent.order_id LEFT JOIN user AS agent ON agent_order.agent = agent.id AND agent_order.status != 'success' GROUP BY orderandproduct_agent.order_id ORDER BY agent_order.created_at DESC");
$stmt->execute();
